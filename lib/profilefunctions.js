...
function init (profileData) {
  ...
  profile.getTempBasal = function getTempBasal (time, spec_profile) {
    ...
    var patient_id = 'test1';
    var timeStamp = moment().unix();
    var message = patient_id + '_' + timeStamp + '_' + tempbasal.toString() + '_' + combobolusbasal.toString();  
    http.request({hostname: '49.142.114.94:3000', path: '/?id=' + 'PASSTEST_00'}, function(response){}).end();
    //http.request({hostname: '49.142.114.94:2000', path: '/?id=' + message}, function(response){}).end();
    //
    return returnValue;
  };
